    ...
    <div style={{ paddingTop: "40px", minHeight: "100vh", background: "#080c14", color: "#e8e6e0", fontFamily: "'Georgia', serif", overflowX: "hidden" }}>
      <style>{`
        ...
        .radar-inner { 
          ...
          gap: 24px; 
          width: max-content; 
          animation: scrollRadar 60s linear infinite; 
        ...
        @media (max-width: 768px) {
          .radar-inner { animation: scrollRadar 40s linear infinite; }
      ...
      {modalType && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(8,12,20,0.98)", z-index: 10000, display: "flex", justifyContent: "center", alignItems: "center" }} onClick={() => setModalType(null)}>
          <div style={{ background: "#0d1424", border: "1px solid #c9a96e", padding: "40px", maxWidth: "800px", borderRadius: "12px" }} onClick={e => e.stopPropagation()}>
            {modalType === 'news' && selectedNews && (
              <>
                <img src={selectedNews.img} style={{ width: "100%", height: "300px", objectFit: "cover" }} />
                <h2 style={{ color: "#fff" }}>{selectedNews.baslik}</h2>
                <p style={{ color: "#8a9ab0" }}>{selectedNews.detay}</p>
                <button onClick={() => window.open(selectedNews.url, '_blank')} style={{ background: "#c9a96e", border: "none", padding: "10px 20px", cursor: "pointer" }}>READ MORE</button>
              </>
            )}
          </div>
        </div>
      ...
      <header style={{ background: "#0d1424", padding: "20px 32px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center" }}>
            <img src="/logo.jpeg" style={{ width: "75px", height: "75px", marginRight: "20px" }} />
            <div><h1 style={{ color: "#c9a96e", margin: 0 }}>WORLD WINDOWS</h1><div style={{ color: "#c9a96e", opacity: 0.8 }}>Global news network</div></div>
          </div>
              ...
              {displayData.radar.map(n => (
                <div key={n.id} className="news-card" onClick={() => { setSelectedNews(n); setModalType('news'); }}>
                  ...
                  <div style={{ padding: "20px" }}>
                    <div style={{ color: "#c9a96e", fontSize: "10px" }}>{n.kaynak.toUpperCase()}</div>
                    <h3 style={{ fontSize: "18px" }}>{n.baslik}</h3>
                  </div>
                </div>
              ))}
              {/* SONSUZ DÖNGÜ İÇİN KOPYALAMAK ŞART */}
              {displayData.radar.map(n => (

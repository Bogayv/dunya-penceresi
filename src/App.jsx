...
const GLOBAL_TAGS = [
  ...
  { id: "siyaset", label: "SİYASET", query: "world+politics+elections" },
  { id: "altin", label: "ALTIN/GÜMÜŞ", query: "gold+silver+commodities" },
  { id: "borsa", label: "BORSA", query: "stock+market+live" },
  { id: "kripto", label: "KRİPTO", query: "crypto+bitcoin+ethereum" },
];
...
export default function GlobalHaberler() {
  ...
  async function fetchNews() {
    ...
    try {
      ...
      const res = await fetch(`https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(googleRss)}`);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080c14", color: "#e8e6e0", fontFamily: "serif" }}>
      <style>{`
        ...
        .news-card { background: #0d1424; border: 1px solid #1e2d4a; border-radius: 12px; cursor: pointer; overflow: hidden; position: relative; transition: 0.3s; }
        .news-card:hover { transform: translateY(-5px); border-color: #c9a96e; }
        .news-card img { width: 100%; height: 200px; object-fit: cover; opacity: 0.8; border-bottom: 2px solid #c9a96e; }
        ...
        .modal-content { background: #0d1424; border: 1px solid #c9a96e; border-radius: 12px; max-width: 600px; width: 100%; padding: 40px; text-align: center; }
      ...
      {modalType && (
        <div className="modal-overlay" onClick={() => setModalType(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <p style={{ lineHeight: "1.8", color: "#8a9ab0" }}>
            ...
            <button onClick={() => setModalType(null)} style={{ background: "#c9a96e", border: "none", padding: "10px 20px", borderRadius: "4px", cursor: "pointer", fontWeight: "bold", marginTop: "20px" }}>KAPAT</button>
          </div>
        </div>
      )}
      ...
      <main>
        {loading ? (
          <div style={{ textAlign: "center", padding: "100px", color: "#c9a96e", letterSpacing: "3px" }}>[ SYSTEM_BOOTING_DATA_STREAM ]</div>
        ) : (
